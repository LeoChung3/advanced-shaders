...
vec3 texturegrab(sampler2D image)
{
	//tri planar

	vec3 blendPercent = normalize(abs(vertexNormal));
	float b = (blendPercent.x + blendPercent.y + blendPercent.z);
	blendPercent = blendPercent / vec3(b);

	vec4 xaxis = texture2D(image, fragmentPos.yz);
	vec4 yaxis = texture2D(image, fragmentPos.xz);
	vec4 zaxis = texture2D(image, fragmentPos.xy);
...
void main()
{
	...
	// specular only      
	//colour = vec4(result * specularStrength, 1.0);

	// has albedo and specular for some reason???
	//gPosition = vec4(result * specularStrength, 1.0) * texture(u_albedoMap, texCoord);
	...
	//made a very rough version of this gradual colour change without looking at the lab sheet,
	//didnt realise it was already showing what i wanted to do
	
	//float val = gPosition.y / 8.f;
	//albedoColour = vec3(1, 1, 1-val);
	//}

	float mid = scale2 *0.5;
	//colours
	vec3 col1 = vec3(0.2, 0.6, 0.1);
	//vec3 col2 = vec3(0.6, 0.2, 0.1);
	
	//merge texture
	//vec3 col2 = texture(u_albedoMap, texCoord*10).rgb;

	//merge texture with traplar
	//vec3 col1 = texturegrab(texture1);
	vec3 col2 = texturegrab(texture2);

	albedoColour = mix(col1, col2, smoothstep(0.0, mid + mid * 0.5, gPosition.y));
