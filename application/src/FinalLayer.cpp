...
FinalLayer::FinalLayer(GLFWWindowImpl& win) : Layer(win)
{
...
void FinalLayer::onRender() const
{
	m_mainRenderer.render();
	m_noiseRenderer.render();
}
void FinalLayer::onUpdate(float timestep)
{
...
void FinalLayer::onKeyPressed(KeyPressedEvent& e)
{
	for (auto it = m_mainScene->m_actors.begin(); it != m_mainScene->m_actors.end(); ++it)
	{
		it->onKeyPress(e);
